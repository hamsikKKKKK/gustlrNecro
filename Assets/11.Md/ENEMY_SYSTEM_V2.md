# Necrocis Enemy System V2 - 핵심 알고리즘 & 구현 정리

> 왜 이 기술이 필요한지, 어떤 원리인지, 실제로 어떻게 적용했는지를 정리한 문서.
> Budget System은 사용하지 않음. Poisson Disk Sampling 기반 스폰.
...
## 이전 Budget System과의 차이

| 항목 | Budget System (V1, 미사용) | Poisson 기반 (V2, 현재) |
|------|---------------------------|------------------------|
| 스폰 위치 결정 | 청크 내 랜덤 위치 | Poisson Disk Sampling (균등 분포) |
| 몹 수량 결정 | 예산(Budget) 기반 가중 랜덤 | density + minDistance로 자동 결정 |
| 설정 방식 | EnemySpawnData (ScriptableObject 별도) | BiomeConfig.enemySpawnRules (통합) |
| 스포너 구조 | EnemyBudgetSpawner (씬 오브젝트) | EnemySpawner (청크 오브젝트, 자동 생성) |
| 활성화 방식 | 청크 로드 시 즉시 스폰 | activationRadius 안에 플레이어 진입 시 스폰 |
| 리스폰 | 없음 (청크 재로드 시만) | respawnCooldown으로 자동 리스폰 |

**왜 바꿨는가**: Budget은 "몇 마리 뽑을까"를 예산으로 제어하지만, 위치 분포가 랜덤이라 뭉침 현상이 발생. Poisson은 위치 균등성을 보장하면서 density로 수량도 자연스럽게 조절됨. 또한 activationRadius 기반이라 플레이어에서 먼 스포너는 비활성 상태로 성능 절약.
...
| 파라미터 | 기본값 | 설명 |
|----------|--------|------|
| heightOffset | 0 | Y축 오프셋 |
| scale | (1,1,1) | 스케일 |
| sortingOrder | 1000 | 렌더링 순서 |
| useBillboard | true | 빌보드 사용 |
| useYSort | true | Y축 정렬 사용 |
| animationSpeed | 0.15 | 애니메이션 속도 |

### Physics (물리)
| 파라미터 | 기본값 | 설명 |
|----------|--------|------|
| addCollider | true | 콜라이더 추가 |
| isTrigger | false | 트리거 여부 |
| colliderSize | (0.7, 1.1, 0.7) | 콜라이더 크기 |
| colliderCenter | (0, 0.55, 0) | 콜라이더 중심 |

### Sprites (스프라이트)
| 파라미터 | 설명 |
|----------|------|
| idleSprites | 대기 애니메이션 프레임 (비면 moveSprites 사용) |
| moveSprites | 이동 애니메이션 프레임 |
...
```csharp
// EnemyController.TryMove()
Vector3 targetPosition = currentPosition + step;
if (biome.CanMove(currentPosition, targetPosition))
{
    MoveToPosition(targetPosition);  // body.position = targetPosition
    return true;
}

// 벽 슬라이딩: X, Z 축 분리 시도
Vector3 moveX = new Vector3(step.x, 0, 0);
Vector3 moveZ = new Vector3(0, 0, step.z);
// 더 큰 축부터 시도 → 자연스러운 벽 미끄러짐
```
